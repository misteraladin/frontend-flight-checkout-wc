...
const returnFlights = reactive(parsedData.segment2 ? JSON.parse(parsedData.segment2) : null);
      ...
      callback: async (action: Action) => {
        if (action === 'confirm') {
          ...
          const data: any = {
            ...
            Segment2: (props.segment2 && props.segment2 !== 'null') ? await JSON.parse(props.segment2) : '',
          ...
          if (response.data.data.Message === 'OK') {
            ...
            ElMessageBox.alert(
              `<div style="display: flex; flex-direction: column; gap: 24px; align-items: center;">
                ${errorAssets}
                <p style="size: 20px; font-weight: 600;">${t('order_cant_processed')}</p>
              </div>`,
              {
                dangerouslyUseHTMLString: true,
